...
{
    public class LectorEditPage
    {
        private readonly IWebDriver _driver;
        private readonly WebDriverWait _wait;

        public LectorEditPage(IWebDriver driver)
        {
            _driver = driver;
        ...
        public void NavigateToLectorEditPage(int userId)
        {
            _driver.Navigate().GoToUrl($"{TestConfig.BaseUrl}/Usuario/Edit/{userId}");
        ...
        public bool IsOnSuccessPage()
        {
            try
            {
                _wait.Until(driver => 
                {
                    ...
                    return currentUrl.Contains("/Usuario") && !currentUrl.Contains("/Edit");
                });
                return true;
            }
        ...
        public bool IsOnEditPage()
        {
            try
            {
                return _driver.Url.Contains("/Usuario/Edit") && 
                       PageTitle.Text.Contains("Editar lector");
            }
            catch (NoSuchElementException)
            {
                return false;
