...
{
    public class LibroEditPage
    {
        private readonly IWebDriver _driver;
        private readonly WebDriverWait _wait;

        public LibroEditPage(IWebDriver driver)
        {
            _driver = driver;
        ...
        // Page actions
        public void NavigateToLibroEditPage(int libroId)
        {
            _driver.Navigate().GoToUrl($"{TestConfig.BaseUrl}/Libro/Edit/{libroId}");
            ...
            try
            {
                var result = _wait.Until(driver => 
                {
                    ...
                    return (currentUrl.Contains("/Libro/Index") || 
                           currentUrl.Contains("/Libro") && !currentUrl.Contains("/Edit")) &&
        ...
        public bool IsOnEditPage()
        {
            try
            {
                return _driver.Url.Contains("/Libro/Edit") && 
                       PageTitle.Text.Contains("Editar libro");
            }
            catch (NoSuchElementException)
            {
                return false;
